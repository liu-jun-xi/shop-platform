# 网店平台 · Cloudflare Workers �?
React 前台 + Cloudflare **Workers**（Hono�? **D1**（结构化数据�? �?**R2** 桶（商品�?/ 用户附件）�?
> �?Express + SQLite 仍保留在 `server/`，可�?`npm run dev:legacy` 跑旧版�?
---

## 一键部署到 Cloudflare

1. 将本仓库推送到 GitHub  
2. 点击下方按钮（把 `liu-jun-xi/shop-platform` 换成你的仓库路径）：
...
图片 URL 仍为 `/uploads/...`，由 Worker 从对�?R2 桶读取�?
---

## 本地开�?
需�?Node.js 18+，并执行�?`npx wrangler login`�?
```bash
npm run install:all
...
�?`.dev.vars.example` 复制�?`.dev.vars` 并填�?`JWT_SECRET`�?
---
...
推送到 `main` / `master` 会触�?[`.github/workflows/deploy.yml`](.github/workflows/deploy.yml)�?
---
...
| 命令 | 说明 |
|------|------|
| `npm run install:all` | 安装�?/ worker / client 依赖 |
| `npm run build` | 构建前端�?`client/dist` |
| `npm run cf:dev` | 本地 Wrangler 开�?|
| `npm run cf:deploy` | 构建并部�?|
...
- Worker 不再使用 `sharp`，上传原图到 R2�?- 备份请用后台「导�?JSON」（�?R2 文件 base64）；不支持系�?zip�?- 生产环境务必更换 `JWT_SECRET` 与默认管理员密码�?